...
{
    public class BannedTribes
    {

        public string tribeName { get; private set; }
        public SolidColorBrush colorBrush { get; private set; }        
        public BitmapImage imgPath { get; private set; }


        public List<BannedTribes> GetBannedTribesList(string _unavaiableTribes, List<TribeAttributes> tribeAttributes, Config _config)
        {
            //HashSet<Race> bannedTribes = tribeAttributes.Where(tribe => !avaiableTribes.Contains(tribe.TribeNr))
            //                 .Select(race => race.TribeNr).ToHashSet();

            
            string[] bannedTribes = _unavaiableTribes.Split(',');
            List<BannedTribes> bannedTribesList = new List<BannedTribes>();

            if (bannedTribes.Length > 0)
            {
                foreach (var bannedTribe in bannedTribes)
                {
                    BannedTribes banned = new BannedTribes();
                    //wenn wieder zurück dann muss hier tribenr stehen.
                    var tribeAttribute = tribeAttributes.Where(tribe => bannedTribe.Trim() == tribe.TribeName).FirstOrDefault();
                    banned.tribeName = tribeAttribute.TribeName;

                    if (_config.showTribeColors == true)
                    {
                        banned.colorBrush = new SolidColorBrush(Color.FromRgb(tribeAttribute.Red, tribeAttribute.Green, tribeAttribute.Blue));
                        // _view.spBanned.Background = solidColorBrush;
                    }
                    if (_config.showTribeImages == true)
                    {
                        if (File.Exists(Config._tribesImageLocation + tribeAttribute.TribeName + @".png"))
                        {
                            banned.imgPath = new BitmapImage(new Uri(Config._tribesImageLocation + tribeAttribute.TribeName + @".png"));
                        }
                    }
                    bannedTribesList.Add(banned);
                }
            }

            return bannedTribesList;
            //else
            //{
            //    //_view.SetisBanned("N/A", "");
            //    if (_config.showTribeColors == true)
            //    {
            //        SolidColorBrush gradientBrush = new SolidColorBrush(Color.FromRgb(0, 0, 0));
        ...
        //private static void SetBannedTribes(View _view, Config _config, TribesOverlay _tribes, Tribe tribeAttribute)
        //{
        //    //_view.SetisBanned(tribeAttribute.TribeName);
        //    if (_config.showTribeColors == true)
        //    {
        //        SolidColorBrush solidColorBrush = new SolidColorBrush(Color.FromRgb(tribeAttribute.Red, tribeAttribute.Green, tribeAttribute.Blue));
        //       // _view.spBanned.Background = solidColorBrush;
        //    }
        //    //if (_config.showTribeImages == true)
        //    //{
        //    //    if (File.Exists(Config._tribesImageLocation + tribeAttribute.TribeName+@".png"))
        //    //    {
        //    //        _tribes.imgTribes.Source = new BitmapImage(new Uri(Config._tribesImageLocation + tribeAttribute.TribeName + @".png"));
        //    //    }
        //    //}
        ...
        //    private static void SetBannedTribes(Guid? gameID, View _view, Config _config,  TribesOverlay _tribes, Tribe tribeAttribute)
        //{
        ...
        //            if (_config.showTribeColors == true)
        //            {
        //                LinearGradientBrush gradientBrush = new LinearGradientBrush(Color.FromRgb(tribeAttribute.Red[0], tribeAttribute.Green, tribeAttribute.Blue), Color.FromRgb(4, 133, 25), new Point(0.5, 0), new Point(0.5, 1));
        //                _view.spBanned.Background = gradientBrush;
        //            }
        //            if (_config.showTribeImages == true)
        //            {
...
//List < Tribe > bannedTribes = tribes.Where(tribe => !gameTribes.Contains(tribe.Type)).ToList();

//if(bannedTribes.Count != 2){
//    // Invalid
//}
//else {
//    ShowBanned(bannedTribes[0], bannedTribes[1]);
//    }

//    public void ShowBanned(tribeA, tribeB, tribeC)
//    {
//        _view.SetisBanned(tribeA.PluralName, tribeB.PluralName, tribeC.PluralName);
//        if (_config.showTribeColors == true)
//        {
//            LinearGradientBrush gradientBrush = new LinearGradientBrush(tribeA.Color, tribeB.Color, tribeC.Color, new Point(0.5, 0), new Point(0.5, 1), new Point(0.5, 2));
//            _view.spBanned.Background = gradientBrush;
//        }

//        if (_config.showTribeImages == true)
//        {
//            string fileName = $"{tribeA.Name.ToLower()}{tribeB.Name.ToLower()}.png";
//            if (File.Exists(Config._tribesImageLocation + fileName))
//            {
//                _tribes.imgTribes.Source = new BitmapImage(new Uri(Config._tribesImageLocation + fileName));
//            }
//        }
//    }
